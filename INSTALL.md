# Sorinote (소리노트) 설치 및 운영 가이드

본 문서는 데비안 12 (Debian 12) 및 Nginx 환경의 서버에 소리노트를 배포하고 안정적으로 운영하기 위한 설치 및 관리법을 상세히 설명합니다.
...
## 6. 데이터베이스 및 미디어 파일 저장 구조 (Data & Storage Architecture)

소리노트의 데이터 및 미디어 리소스(이미지, 사운드 녹음 파일 등)가 어떻게 저장되고 관리되는지 설명합니다. 백업이나 용량 관리 시 이 구조를 참고하시기 바랍니다.

### 6-1. 파일 저장 위치 (Storage Location)
* **데이터베이스 (SQLite)**: 
  * 로컬 PM2 배포 시: [server/database.sqlite](file:///Users/2b/Desktop/sorinote/server/database.sqlite) 경로에 단일 파일 형태로 보관됩니다.
  * 도커 배포 시: 컨테이너 볼륨 바인딩을 통해 프로젝트 루트의 `./data/database.sqlite`에 보관됩니다.
* **미디어 업로드 폴더 (Uploads Directory)**: 
  * 사용자가 업로드한 오디오 녹음 파일(mp3, wav 등) 및 이미지 파일(png, jpg 등)은 서버의 다음 폴더에 물리적으로 저장됩니다.
  * 로컬 PM2 배포 시: `server/uploads/` 폴더 내에 저장됩니다. (환경 변수 `UPLOAD_DIR`로 위치 변경 가능)
  * 도커 배포 시: 프로젝트 루트의 `./data/uploads/` 폴더 내에 저장됩니다.

### 6-2. 파일 저장 명명 규칙 (Filename Convention)
업로드되는 모든 사운드 및 이미지 파일은 파일명 충돌을 방지하고 보안을 확보하기 위해 서버에 저장될 때 고유한 이름으로 자동 변경됩니다.
* **명명 규칙**: `[타임스탬프(밀리초)]-[9자리 난수].[원래 확장자]`
  * 예: `1718698000000-482910582.mp3`
* 이 파일명은 데이터베이스의 `memos` 테이블 내 `audioUrl` 혹은 `imageUrl` 필드에 `/uploads/1718698000000-482910582.mp3` 형태로 등록되며, 클라이언트는 이 웹 경로를 통해 파일에 접근합니다.

### 6-3. 데이터 백업 방법 (Data Backup)
서버 데이터 전체를 안전하게 백업하려면 다음 요소만 백업 디렉토리에 복사하거나 아카이브하면 됩니다.
* **PM2 배포 환경**:
  * 데이터베이스 파일: `/home/banggumail/sorinote/server/database.sqlite`
  * 업로드 미디어 폴더: `/home/banggumail/sorinote/server/uploads/`
* **도커 배포 환경**:
  * 프로젝트 루트의 `./data/` 디렉토리 전체 (안에 DB 파일과 `uploads/` 폴더가 모두 포함되어 있습니다).

---

## 7. 포트 번호 변경 방법 (Port Configuration)
...
## 8. 최대 업로드 용량 제한 설정 및 업데이트 방법 (Upload Size Limit Configuration)
...
## 9. 자동 리소스 정리 및 용량 관리 설정 (Automated Garbage Collection)
